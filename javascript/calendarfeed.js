...
sakai_global.calendarfeed = function(tuid, showSettings) {
    ...
    var LIGHT_HEARTED_ERROR_TITLES = [
        "Fiddlesticks!", "Oh dear…", "Dagnabbit!", "Oops…", 
        "What A Kerfuffle!"
    ];
    ...
    var settingsFormTitleField = 
        $("#calendarfeed_settings_txtTitle", $rootel);
    ...
    function compareEventByStartDate(a, b) {
    ...
    function getState(callback) {
        ...
        sakai.api.Widgets.loadWidgetData(tuid, defaultingStateLoadHandler(callback, {
            unconfigured: true,
            title: "",
            url: "",
    ...
    function updateCalendar() {
        ...
        $(".ajax-content .summary.compact", $rootel).toggle(expandCalendarEntry, contractCalendarEntry);
    ...
    function expandCalendarEntry(e) {
    ...
    function contractCalendarEntry(e) {
        var summary = $(e.target);
        var expanded = summary.siblings(".full");
    ...
    function onWidgetSettingsDataSaved(success, data) {
        if (success) {
            ...
            sakai.api.Util.notification.show("Couldn't Save Your Settings", "An error prevented your settings from being saved. " +
            " Please try again.", sakai.api.Util.notification.type.ERROR);
        }
    }
    ...
    function settingsHandleRangeSlide(event, ui) {
        ...
        var fromString = !isFinite(from) ? "any date in the past" : buildVeryRelativeDateString(from);
        var toString = !isFinite(to) ? "any date in the future" : buildVeryRelativeDateString(to);
        
        $("#calendarfeed_settings_daterangeslider_label .from", $rootel).text(fromString);
        $("#calendarfeed_settings_daterangeslider_label .to", $rootel).text(toString);
    }
